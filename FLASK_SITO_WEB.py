...
import random
app = Flask(__name__)
lista_fatti =["La maggior parte delle persone che soffrono di dipendenza tecnologica sperimentano un forte stress quando si trovano al di fuori dell'area di copertura della rete o non possono utilizzare i loro dispositivi",
...
@app.route("/")
def index():
    return f'<h1> Come state? Io bene </h1> <a href="/fatto_casuale">Scopri dei fatti interessanti'
@app.route("/fatto_casuale")
def fatti_dipendeza_social():
    return f'<h2> Ecco un fatto interessante casuale sulla dipendenza ai social </h2>'
    return f'<p>{random.choice(lista_fatti)}</p>'
    return f'<p> Ricarica la pagina se vuoi scoprire altri fatti interessanti </p>'
app.run(debug=True)
